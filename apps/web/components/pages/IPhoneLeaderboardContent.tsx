...
const IPHONE_OFFER_ID = 'bfe67087-50ed-4977-99be-f000628608ec';
...
export default function IPhoneLeaderboardContent() {
    ...
    <>
      ...
      <main className="min-h-screen bg-background pt-24 pb-16">
        <div className="container-custom px-4">
          {/* Back Link */}
          <Link href="/offers/iphone" className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground mb-8 transition-colors">
            <ArrowLeft className="w-4 h-4" />Back to Offer Details
          </Link>

          {/* Hero */}
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-accent/10 text-accent rounded-full text-sm mb-6">
              <Trophy className="w-4 h-4" />iPhone Draw
            </div>
            <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-4">Qualified Entrants</h1>
            <p className="text-lg text-muted-foreground max-w-xl mx-auto">
              Everyone listed below has qualified for the iPhone draw by enrolling in our 3-month personal training program.
            </p>
          </div>
          ...
          <div className="flex justify-center gap-8 mb-12">
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 text-4xl font-bold text-accent mb-2">
                <Users className="w-8 h-8" />{entrants.length}
              </div>
          ...
          {!isLoading && entrants.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground mb-6">Be the first to join! Sign up for our 3-month personal training program.</p>
              <Button asChild>
                <Link href="/offers/iphone">Learn More &amp; Enter</Link>
              </Button>
            </div>
          )}
          ...
          {!isLoading && entrants.length > 0 && (
            <div className="text-center mt-12">
              <p className="text-muted-foreground mb-4">Want to join the draw? Sign up for 3 months of personal training!</p>
              <Button asChild size="lg">
                <Link href="/offers/iphone">Enter the Draw</Link>
              </Button>
            </div>
          )}
        </div>
